...
def bond_finder_0(brackets):
    ...
    while True:
        ...
        found = False
        while not found:
            index_to_index = list(range(0, len(index) - 1))
            for i in index_to_index:
                if brackets[index[i]] == '(' and brackets[index[i + 1]] == '.':
                    try:
                        for ii in range(i + 1, len(index) - 1):
                            if brackets[index[ii]] == '(':
                                kill
                            bond = []
                            if brackets[index[ii]] == ')':
                                bond.append(index[i])
                                bond.append(index[ii])
                                bonds.append(bond)
                                index.remove(index[i])
                                index.remove(index[ii - 1])
                                found = True
                                break
                        if found:
                            break
...
def bond_grapher_2(abcissa, linear, bonds, color, gradient_0, gradient_color_graph, color_spectrum,
                 mutation_imposition, alpha_0, mosaic, bond_ubiquity):
    ...
    if mosaic:
        for i,j in enumerate(bonds):
            s = (bonds[i][0]+bonds[i][1]) / 2
            r = abs(bonds[i][0]-bonds[i][1]) / 2
            final_ordinate = []
            for ii in abcissa:
                try:
                    final_ordinate.append(math.sqrt((r ** 2) -(ii-s) ** 2))
...
#Description: Generates a list of lists of the form ['sequence_name_from_fasta', ['NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN.....']]
	#by using a multiple sequence alignment output from ClustalW
#Input: path to MSA alignment
#Output: list of lists of the form ['sequence_name_from_fasta','NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN.......']
def MSA_alignment_extraction(path_to_MSA_file):
    MSA_txt=open(path_to_MSA_file)
    MSA_txt=MSA_txt.readlines()
    MSA_txt=[i.split(' ') for i in MSA_txt]
    test_MSA=MSA_txt[0]
    MSA_txt=[[i for i in ii if i != ''] for ii in MSA_txt]
    MSA_txt=[i for i in MSA_txt if len(i) == 2 and '*' not in i[0]]

    for i,j in enumerate(MSA_txt):
        MSA_txt[i][1]=MSA_txt[i][1].rstrip('\n')

    serials=[]
    for i in MSA_txt:
        if i[0] not in serials:
            serials.append(i[0])

    serials=[[i,[]] for i in serials]

    for i,j in enumerate(serials):
        for ii in MSA_txt:
            if serials[i][0] == ii[0]:
                serials[i][1].append(ii[1])

    for i in range(0,len(serials)):
        serials[i][1]=''.join(serials[i][1])
